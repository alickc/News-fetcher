...
def getNews(link, c):
    ...
    def getTime():
        date = driver.find_elements_by_css_selector('time')[0].get_attribute("datetime")
        return date
...
def save(header, paragraph, date, c, link):
    writer = "LINK:" + link + "\nTITLE: " + header + "\n" + paragraph
    if len(paragraph) > 10:
        with open(constants.path + "independent/[" + str(c) +"]" + date[:-5].replace(":","-") + ".txt", "a+", encoding="utf-8") as f:
            f.write(writer)
            print("saved: " + constants.path + "independent/[" + str(c) +"]" + date[:-5].replace(":","-") + ".txt")
