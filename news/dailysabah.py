...
def getNews(link, c):
    ...
    def getTime():
        date = driver.find_element_by_class_name("right_mobile_details").text
        return date
...
def save(header, paragraph, date, c, link):
    writer = "LINK:" + link + "\nTITLE: " + header + "\n" + paragraph
    if len(paragraph) > 10:
        with open(constants.path + "dailysabah/[" + str(c) + "]" + date[:-1].replace(":", "-") + ".txt", "a+",
                  encoding="utf-8") as f:
            f.write(writer)
            print("saved: " + constants.path + "dailysabah/[" + str(c) + "]" + date[:-1].replace(":", "-") + ".txt")